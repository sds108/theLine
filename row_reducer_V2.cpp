...
long double A_Lift = 6;
long double A_Drag = 2;
...
int main() {
	...
	A_LR_LF = atan2(d2 + d3, d7 - d9);
	
	R_LR_COM = sqrt(pow(d3, 2) + pow(d7, 2));
	R_LR_COP = sqrt(pow(d3 + d4, 2) + pow(d7 + d10, 2));
	R_LR_LF = sqrt(pow(d2 + d3, 2) + pow(d7 - d9, 2));
	
	cout << "A_LF_RR " << A_LF_RR << endl;
	cout << "A_RR_COM " << A_RR_COM << endl;
	cout << "A_RR_COP " << A_RR_COP << endl;
	cout << "A_RR_RF " << A_RR_RF << endl;
	
	cout << "A_RF_LR " << A_RF_LR << endl;
	cout << "A_LR_COM " << A_LR_COM << endl;
	cout << "A_LR_COP " << A_LR_COP << endl;
	cout << "A_LR_LF " << A_LR_LF << endl;
	
	cout << "R_RR_COM " << R_RR_COM << endl;
	cout << "R_RR_COP " << R_RR_COP << endl;
	cout << "R_RR_RF " << R_RR_RF << endl;
	
	cout << "R_LR_COM " << R_LR_COM << endl;
	cout << "R_LR_COP " << R_LR_COP << endl;
	cout << "R_LR_LF " << R_LR_LF << endl;
	...
	*/
	...
	mat[0][2] = 1;
	mat[0][3] = 1;
	mat[0][4] = m * g - ((omega2 * rho * C_Lift * A_Lift * R_COP * R_COP * pow(cos(A_COP), 2)) / 2);
	...
	*/
	...
	mat[1][0] = -mew * cos(abs(A_COM - A_LF));
	mat[1][1] = -mew * cos(abs(A_COM - A_RF));
	mat[1][2] = -mew * cos(abs(A_COM));
	mat[1][3] = -mew * cos(abs(A_COM));
	mat[1][4] = - omega2 * m * R_COM;
	//*/
	...
	/*
	// Equation 1, M under COM at side = 0
	// omega^2 * (rho * R_COP^2 * cos(theta_COP)^2 * (d_5 * C_Drag * A_Drag + d_4 * C_Lift * A_Lift) - d_1 * m * R_COM * sin(theta_COM)) / 2 + d_2 * (N_LF + N_RF) - d_3 * (N_LR + N_RR) = 0
	mat[1][0] = d2;
	mat[1][1] = d2;
	mat[1][2] = -d3;
	mat[1][3] = -d3;
	mat[1][4] = -omega2 * (((rho *  R_COP * R_COP * pow(cos(A_COP), 2) * (d5 * C_Drag * A_Drag + d4 * C_Lift * A_Lift)) / 2) - d1 * m * R_COM * sin(A_COM));
	//*/
	
	///*
	// Equation 1, M under Front at side = 0
	mat[1][0] = 0;
	mat[1][1] = 0;
	mat[1][2] = -(d2 + d3);
	mat[1][3] = -(d2 + d3);
	mat[1][4] = -omega2 * (((rho *  R_COP * R_COP * pow(cos(A_COP), 2) * ((d5 * C_Drag * A_Drag) - ((d2 - d4) * C_Lift * A_Lift))) / 2) - d1 * m * R_COM * sin(A_COM)) - d2 * m * g;
	//*/
	
	// Equation 2, M around RF to LR = 0
	mat[2][0] = R_LR_LF * sin(A_LR_LF - A_RF_LR);
	mat[2][1] = 0;
	mat[2][2] = 0;
	mat[2][3] = -(d7 + d6) * sin(A_RF_LR);
	mat[2][4] = omega2 * ((((rho *  R_COP * R_COP * pow(cos(A_COP), 2)) * (C_Lift * A_Lift * R_LR_COP * sin(A_RF_LR - A_LR_COP) - C_Drag * A_Drag * d5 * sin((PI / 2) - A_RF_LR))) / 2) - d1 * m * R_COM * sin((PI - A_COM) - A_RF_LR)) - m * g * R_LR_COM * sin(A_RF_LR - A_LR_COM);
	
	/* Cancelling out
	// Equation 3, M diagonal from LF to RR = 0
	mat[3][0] = 0;
	mat[3][1] = -R_RR_RF * sin(A_RR_RF - A_LF_RR);
	mat[3][2] = (d7 + d6) * sin(A_LF_RR);
	mat[3][3] = 0;
	mat[3][4] = R_RR_COM * m * g * sin(A_LF_RR - A_RR_COM) - omega2 * ((((rho *  R_COP * R_COP * pow(cos(A_COP), 2)) * (C_Lift * A_Lift * R_RR_COP * sin(A_LF_RR - A_RR_COP) - C_Drag * A_Drag * d5 * sin((PI / 2) - A_LF_RR))) / 2) - d1 * m * R_COM * sin(A_LF_RR - A_COM));
	//*/
	...
	mat[3][2] = -d7;
	mat[3][3] = d6;
	mat[3][4] = -omega2 * (((rho *  R_COP * R_COP * pow(cos(A_COP), 2) * d10 * C_Lift * A_Lift) / 2) + d1 * m * R_COM * cos(A_COM));
	//*/
	...
	*/
	...
	cout << "Speed = " << (linear_velocity * 3600) / 1000 << " km / hr\n\n";
